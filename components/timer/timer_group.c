...
double TIMER_INTERVAL0_SEC = 0;   /*!< test interval for timer 0 */
double TIMER_INTERVAL1_SEC = 0;   /*!< test interval for timer 1 */

#define TETIMER0   0   /*!< example of auto-reload mode */
#define TETIMER1   1      /*!< example without auto-reload mode */
...
PF_TIMER_GROUP *g_timer_group_cb=NULL;
...
void timer_evt_task(void *arg)
{
    while(1) {
        ...
        if(evt.type == TETIMER0) {
			g_timer_group_cb(0);
        } else if(evt.type == TETIMER1) {
			g_timer_group_cb(1);
        }
		/*
...
void timer_group_init(PF_TIMER_GROUP* pf,float time0_sec,float time1_sec)
{
	...
	if(time0_sec>0)
	{
		TIMER_INTERVAL0_SEC=time0_sec;
		example_tg0_timer0_init();
	}
	if(time1_sec)
	{
		TIMER_INTERVAL1_SEC=time1_sec;
