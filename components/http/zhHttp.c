...
bool zhHttpGet(const char*szUrl,int begin_byte,int timeout_second,ZH_ONHTTPDATA* pfCallback)
{
	...
	//开启线程
	xTaskCreate(_zhHttpThread_Data, "httpgf0", 8192, threadParm, 5, NULL);
	return ret;
}
...
bool zhHttpPost(const char*szUrl,const char*body,int body_len,int begin_byte,int timeout_second,ZH_ONHTTPDATA* pfCallback)
{
	...
	//开启线程
	xTaskCreate(_zhHttpThread_Data, "httpgf1", 8192, threadParm, 5, NULL);
	return ret;
}
...
bool zhHttpSize(const char*szUrl,int timeout_second,ZH_ONHTTPDATA* pfCallback)
{
	...
	xTaskCreate(_zhHttpThread_Head, "httpgf2", 8192, threadParm, 5, NULL);
	return ret;
}
...
_data_ok:
	...
	while(1)
	{
		recv_len=zhSockRecv(recv_buf,2047);
		if(recv_len>0)
		{
			recv_buf[recv_len]=0;
